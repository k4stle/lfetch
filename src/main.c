...
#define INFO_LINES 13

// gets distro from file
const char *get_distro(void) {
...
const char *get_uptime(void) {
  ...
  if (days > 0)
    snprintf(output, sizeof(output), "%ldd %02ldh %02ldm", days, hours,
             minutes);
  else if (hours > 0)
    snprintf(output, sizeof(output), "%ldh %02ldm", hours, minutes);
  else
    snprintf(output, sizeof(output), "%ldm", minutes);
...
int main(void) {
  ...
  snprintf(ascii_path, sizeof(ascii_path), "%s/ascii.txt",
           exe_path); // makes sure that the ascii art file must be in the
                      // directory as the build file
  ...
  while (total_art < INFO_LINES && fgets(buf, sizeof(buf), ascii) != NULL) {

    buf[strcspn(buf, "\n")] =
        '\0'; // makes sure the ascii art and the info arent colliding

    art_lines[total_art] =
        strdup(buf); // stores the amount of lines in the array
  ...
  for (int line = 0; line < INFO_LINES; line++) {
    printf("\033[31m%-20s\033[0m", line < total_art ? art_lines[line] : "");
